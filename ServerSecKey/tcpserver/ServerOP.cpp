...
#include "../protobuf/RespondCodec.h"
#include "../protobuf/RespondFactory.h"
#include "TcpSocket.h"
#include "../protobuf/Message.pb.h"
#include "../openssl/RsaCrypto.h"
#include "../openssl/Hash.h"
#include "../protobuf/RequestFactory.h"
#include "../protobuf/RequestCodec.h"
#include <unistd.h>
#include "../shm/BaseShm.h"
...
void ServerOP::startServer()
{
	...
	while (1)
	{
		logAddr->Log("等待客户端连接...", __FILE__, __LINE__);
		...
		logAddr->Log("客户端连接成功...", __FILE__, __LINE__);
...
string ServerOP::secKeyAgree(RequestMsg* req)
{
	...
	if (b != true)
	{
		...
		logAddr->Log("数据校验失败...", __FILE__, __LINE__);

		//cout << "数据校验失败...." << endl;
	}
	else
	{
		logAddr->Log("校验签名成功..", __FILE__, __LINE__);
		//cout << "校验签名成功..." << endl;
...
string ServerOP::secKeyJiaoYan(RequestMsg* req)
{
	...
	{
		reqInfo.status = true;
		logAddr->Log("秘钥校验成功....", __FILE__, __LINE__);
	...
	{
		reqInfo.status = false;
		logAddr->Log("秘钥校验失败....", __FILE__, __LINE__);
...
string ServerOP::secKeyZhuxiao(RequestMsg* req)
{
	...
	if (b)
	{
		...
		res.status = true;
		logAddr->Log("秘钥注销成功...", __FILE__, __LINE__);
	...
	else
	{
		res.seckeyID = SecKeyId;
		res.status = false;
		logAddr->Log("秘钥注销失败...", __FILE__, __LINE__);
